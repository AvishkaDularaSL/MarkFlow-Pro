...
router.post('/execute', AuthService.requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  ...
  else if (outputFormat === 'jpeg') fileExt = '.jpg';
  else if (outputFormat === 'avif') fileExt = '.avif';
  ...
  for (let i = 0; i < originalImages.length; i++) {
    ...
    try {
      ...
      const outputFilename = `${safeBase}_watermarked${fileExt}`;
      const outputPath = path.join(processedDir, `${orig.id}_${outputFilename}`);

      const result = await ImageProcessingService.processImage(
        orig.temporary_path,
        business.logo_path,
        { ...config, output_format: outputFormat, quality: effectiveQuality },
        outputPath,
        outputFilename
      );
...
// 8. Secure download individual processed image (WebP, PNG, JPEG, AVIF)
router.get('/download/image/:id', AuthService.requireAuth, (req: AuthenticatedRequest, res: Response) => {
  ...
  res.setHeader(
    'Content-Disposition',
    `attachment; filename="${encodeURIComponent(processed.output_filename)}"`
  );
  fs.createReadStream(processed.output_path).pipe(res);
});

// 9. Secure download batch ZIP archive
router.get('/download/zip/:jobId', AuthService.requireAuth, (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user!.id;
  ...
  if (!job.zip_path || !fs.existsSync(job.zip_path)) {
    return res.status(404).json({ error: 'ZIP archive is expired or not ready.' });
  }
